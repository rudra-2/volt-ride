...
def start_ride(ride_id):
    ...
    print(f"Start Ride API called with ride_id: {ride_id}")
    cursor.execute("SELECT * FROM ride WHERE `r-id` = %s", (ride_id,))
    ride = cursor.fetchone()
    vehicle_id = ride['v-id']
    # Perform necessary logic to start the ride
    # Example: Insert ride details into the database
...
def end_ride(ride_id):
    try:
        ...
        cursor.execute("UPDATE vehicle SET status = 'Available' WHERE `v-id` = %s;", (ride_id,))
        ...
        cursor.execute("""
            UPDATE ride
            SET `payment`=%s, `profit`=%s, 
            WHERE `r-id`=%s;
        """, (total_payment, total_profit, ride['r-id']))
        db.commit()
